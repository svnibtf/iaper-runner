...
function GetDefaultValue($field, $ptype, $table="")
{
	...
	if(!$table)
		$table=$strTableName;
				if($table=="calcalendar" && $field=="DateField")
	{
		return $_SESSION["yr"]."-".$_SESSION["mon"]."-".$_SESSION["days"];
	}
				if($table=="calcalendar" && $field=="EndDate")
	{
		return ($_SESSION["yrEnd"])."-".$_SESSION["monEnd"]."-".$_SESSION["daysEnd"];
	}
				if($table=="calcalendar" && $field=="Period")
	{
		return "d";
	}
				if($table=="calcalendar" && $field=="PeriodDays")
	{
		return setDefaultPeriodDay();;
	}
				if($table=="calcalendar" && $field=="RecurType")
	{
		return "Never";
	}
				if($table=="calcalendar" && $field=="RecurAfterValue")
	{
		return 1;
	}
				if($table=="calglobals" && $field=="TimePeriod")
	{
		return 30;
	}
				if($table=="calglobals" && $field=="FirstDayOfWeek")
	{
		return 0;
	}
				if($table=="calglobals" && $field=="EditInPast")
	{
		return 0;
	}
	return "";
        ...
        if (isset($call['args']))
        {
            ...
            foreach ($call['args'] as $arg)
            {
                ...
                if (is_array($arg))
                {
                    ...
                    $arrStr = strlen($arrStr) < $paramMaxDisplay ? $arrStr : substr($arrStr, 0, $paramMaxDisplay).'...';
                ...
                {
                	$arg = @strlen($arg) < $paramMaxDisplay ? $arg : @runner_htmlspecialchars(substr($arg, 0, $paramMaxDisplay)).'...';
                    $args[] = $j.'.&nbsp;'.$arg.';';
                }
            }
