		...
		events = JSON.parse(txt);
		 $('#calendar').fullCalendar({
			...
			viewRender: function (view, element) {
				...
				if(view.name=="year" && proxy["lang"]=="he"){
					$("tr.fc-year-week-days").each(function(){
						v = $(this).find("th").eq(0).html();
						$(this).find("th").eq(0).html($(this).find("th").eq(6).html());
						$(this).find("th").eq(6).html(v);
						v = $(this).find("th").eq(1).html();
						$(this).find("th").eq(1).html($(this).find("th").eq(5).html());
						$(this).find("th").eq(5).html(v);
						v = $(this).find("th").eq(2).html();
						$(this).find("th").eq(2).html($(this).find("th").eq(4).html());
						$(this).find("th").eq(4).html(v);
...
if(ctrlCategory){
	$("div[data-fieldname=Category]").attr("style", "background-color:" + catLookup[ ctrlCategory.getValue() ]);
...
if(!pageObj.editable){
	pageObj.toggleItem("edit_button", false );
	pageObj.toggleItem("delete_button", false );
}
;
});
