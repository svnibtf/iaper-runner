...
function calendar_getGlobalSettings(){
	...
	if ($dataCalglobals = db_fetch_array($rsCalglobals)) {
		...
		if(!$_SESSION["calendar_StartTime"])
			$_SESSION["calendar_StartTime"] = "00:30:00";
		if(!$_SESSION["calendar_EndTime"])
			$_SESSION["calendar_EndTime"] = "00:00:00";
	}
...
function calendar_getListOfEventsInRange($rangeFrom, $rangeTo, $searchWhereClause, $connection)
{
	...
	$whereOwnerID = "1=1";
	if( $strPerm==1 && $_SESSION["AccessLevel"] != ACCESS_LEVEL_ADMINGROUP)		
		$whereOwnerID = $connection->addFieldWrappers($pSet->getTableOwnerID())."='".$pSet->getCaseSensitiveUsername((string)$_SESSION["_calcalendar_OwnerID"])."'";

	$sql = "select " . $calendarTable . ".* ".$sqlCategoryField.
			" from " . $calendarTable . 
			$sqlCategoryJoin . 
			" where (" . $searchWhereClause . ") and (" . 
			$dateField . " >= " . $fromDate . " and " . $dateField . " < " . $toDate . 
			$sqlRecurrence .  " ) and (".$whereOwnerID.")" .
			$sqlTimeField;
	...
	while( $data = $result->fetchAssoc() )
	{
		...
		if(!$endDate)
			$endDate=$startDate;
		// && CheckSecurity($data["OwnerID"], "Edit", "caldaily")
...
function calendar_getEventHTML($event, $fireDbDate, $allowEditInPast, $pageType="list", $pageName="")
{
	...
	$draggableValue = "";
	$draggableClass = "";
	if ($pageName == "monthly" && $_SESSION["UserID"]!="Guest" && CheckTablePermissions("calmonthly","E") && CheckSecurity($event["OwnerID"], "Edit", "calmonthly") ){
	...
	{		
		if ($pageType != "print" || $pageName != "yearly") {
			if ($_SESSION["UserID"]=="Guest" || $eventInThePast && !$allowEditInPast || !CheckTablePermissions("calmonthly","E") || !CheckTablePermissions("calweekly","E") || !CheckSecurity($event["OwnerID"], "Edit", "calmonthly")) 
...
function _calendar_getListOfDatesInRange($rangeFrom, $rangeTo, $searchWhereClause, $connection)
{
	...
	while( $data = $result->fetchAssoc() )
	{
		...
		$startDate = db2time( $data[ calendarGetSetting(calendarGetDateOrTimeKey("date"))] );
		$endDate = db2time( $data[ calendarGetSetting("calendarEndDate") ] );
		if( calendar_isEventInRange($rangeFrom, $rangeTo, $startDate, $endDate, $period ) && (CheckSecurity($data[$pSet->getTableOwnerID()], "Edit", "calcalendar") || CheckSecurity($data[$pSet->getTableOwnerID()], "Search", "calcalendar")) )
			$output[] = $data;
	}
	return $output;
}
	...
	if( isset($event) && count($event) )
	{
		...
		if( count($event) > $_SESSION["calendar_CountEvents"] ) 
		{
			$eventsHTML .= "<a href='#' id=moreButton".$fireDbDate[2]." yr=".$fireDbDate[0]. " mon=" . $fireDbDate[1] 
				. " days=" .$fireDbDate[2].">More... </a> (Total: ".count($event).")";
		}
...
function calendar_tempValidateEvent(&$values, &$ret, &$message)
{
	...
	else
	{
		...
		{
			$message="<span class=message calendar_error>Invalid event date or time</span>";
			$ret=false;
		}
		else if (calendarFieldIsset(calendarGetSetting("calendarEndTime")) && comparedates($dbDateTimeEnd, $dbDateTime)<0) {
			$message="<span class=message calendar_error>Invalid event time</span>";
			$ret=false;
		}
		else if (calendarFieldIsset(calendarGetSetting("calendarEndDate")) && comparedates( $dbEndDate, $dbDate) < 0 )
		{
			$message="<span  class=message calendar_error>Invalid event end date or time</span>";
			$ret=false;
		}
		...
		{
			$message="<span class=message calendar_error>Select the day of the week</span>";
			$ret=false;
		}
		else 
		{
...
function calendar_outputTimeFieldsEdit($recID){
	...
	for ($i=0; $i<(1440/$selectTimePeriod);$i++)
	{
		...
		if(strtotime($_SESSION["calendar_StartTime"])>strtotime(calendar_time2db($timeArray)) && calendar_time2db($timeArray)!="00:00:00")
			$isShow = false;
		if(strtotime($_SESSION["calendar_EndTime"])<strtotime(calendar_time2db($timeArray)) && calendar_time2db($timeArray)!="00:00:00" && $_SESSION["calendar_EndTime"]!="00:00:00")
			$isShow = false;
		if(calendar_time2db($timeArray)=="00:00:00" && $_SESSION["calendar_EndTime"]!="00:00:00")
			$isShow = false;
		...
		if($isShow)	{
			...
			if (comparedates($timeArray,$dbDateTimeEnd)==0 && $selectNumberET == 0){
					$selectNumberET = $j+1;	
			}
			$j++;
	...
	$selectNumberTF = 0;
	$selectNumberET = 0;
	echo "var gtimes=[{value: '',display:'Please select'}";
	$j=0;
	for ($i=0; $i<(1440/$selectTimePeriod);$i++)
	{
		$timeArray = addMinutes($timeArray, $selectTimePeriod); 
	...
	if($selectNumberET==0)
		$selectNumberET = 48;
	echo "];";
	echo "var gTimeField=".$selectNumberTF.";";
	echo "var gEndTime=".$selectNumberET.";";
...
function calendar_getDataOfCategoryLookup(){
	...
	$CategoryRS = $dal_calcategory->QueryAll();
	
	$retOutput = "var catLookup = [];";
	while ($CategoryData = db_fetch_array($CategoryRS)) {
	...
	foreach ($valuesRecord as $key=>$val){
		...
		if (($key!="DateField") || ($key!="TimeField")) {
			if (calendarGetSetting("calendar".$key)!="")
				$tblCalendar->Value[calendarGetSetting("calendar".$key)] = $val;
			else
				$tblCalendar->Value[$key] = $val;
		}
	}
	if (calendarGetDateOrTimeKey("date")=="calendarDateTime"){
		$tblCalendar->Value[calendarGetSetting("calendarDateTime")] = date("Y-m-d",strtotime($valuesRecord["DateField"]))." ".$valuesRecord["TimeField"];
	}
	else {
		$tblCalendar->Value[calendarGetSetting("calendarDateField")] = $valuesRecord["DateField"];
		$tblCalendar->Value[calendarGetSetting("calendarTimeField")] = $valuesRecord["TimeField"];
	}
	$tblCalendar->Add();
}
...
function calendar_UpdateRecord($valuesRecord, $oldvaluesRecord, $keysRecord){
	...
	foreach ($keysRecord as $key1=>$val1){
		$tblCalendar->Param[calendarGetSetting("calendar".$key1)] = $val1;
	}
	...
	foreach ($valuesRecord as $key2=>$val2){
		if (($key2!="DateField") && ($key2!="TimeField")) {
			if (calendarGetSetting("calendar".$key2)!="")
				$tblCalendar->Value[calendarGetSetting("calendar".$key2)] = $val2;
			else
				$tblCalendar->Value[$key2] = $val2;
		}
		else {
			if (calendarGetDateOrTimeKey("date")=="calendarDateTime"){
				$tblCalendar->Value[calendarGetSetting("calendarDateTime")] = date("Y-m-d",strtotime($valuesRecord["DateField"]))." ".$valuesRecord["TimeField"];
...
function calendar_getMaxDateField($connection){
	...
	$dateField = $connection->addFieldWrappers(calendarGetSetting(calendarGetDateOrTimeKey("date")));
	$sql = "select max(" . $tblCalendar . ".".$dateField.") as maxdate " . 
		" from " . $tblCalendar;
...
function calendar_getMinDateField($connection){
	$tblCalendar = $connection->addTableWrappers(calendarGetSetting("calendarTable"));
	$dateField = $connection->addFieldWrappers(calendarGetSetting(calendarGetDateOrTimeKey("date")));
	$sql = "select min(" . $tblCalendar . ".".$dateField.") as mindate " . 
		" from " . $tblCalendar;
