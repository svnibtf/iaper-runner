...
function normalized_date_format() 
{
	global $locale_info;
	$elements = array( "dd", "MM", "yyyy");
	if( $locale_info["LOCALE_IDATE"] == "0" )
	{
		$elements = array( "MM", "dd", "yyyy" );
	}
	else if( $locale_info["LOCALE_IDATE"] == "2" )
	{
		$elements = array( "yyyy", "MM", "dd" );
	}
	return implode( $locale_info["LOCALE_SDATE"], $elements );
	
}

function format_normalized_shortdate($time)
{
	return format_datetime_custom($time, normalized_date_format() );
}
