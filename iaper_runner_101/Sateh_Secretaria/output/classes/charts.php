...
//				$this->strLabel = $params['name'];
	...
	protected function isProjectDB()
	{
		...
		if("adm_agenda_1" == $this->chrt_array['tables'][0])
			return true;
		if("adm_pacientes_documentos" == $this->chrt_array['tables'][0])
			return true;
		if("adm_tipo_documentos" == $this->chrt_array['tables'][0])
			return true;
		if("adm_pagamento_avulso" == $this->chrt_array['tables'][0])
			return true;
		if("adm_campos_adicionais" == $this->chrt_array['tables'][0])
			return true;
		if("adm_campos_paciente" == $this->chrt_array['tables'][0])
			return true;
		if("adm_splits" == $this->chrt_array['tables'][0])
			return true;
		if("adm_eventos" == $this->chrt_array['tables'][0])
			return true;
		if("adm_lista_variavel" == $this->chrt_array['tables'][0])
			return true;
		if("adm_inscricoes" == $this->chrt_array['tables'][0])
			return true;
		return false;
	}
