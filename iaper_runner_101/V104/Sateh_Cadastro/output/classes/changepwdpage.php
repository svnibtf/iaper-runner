...
class ChangePasswordPage extends RunnerPage
{
	...
	protected function getSelectSQL( $where )
	{
		...
		if( $this->pSet->usersTableInProject() )
			$strSQL = "select ".$this->getFieldSQLDecrypt( $this->passwordField ) .','. $this->getFieldSQLDecrypt( $this->usernameField );
		else
			$strSQL = "select ".$this->connection->addFieldWrappers( $this->passwordField ) .','. $this->connection->addFieldWrappers( $this->usernameField );	
	...
	protected function changePassword()
	{
		...
		$data = array( $cPasswordField => $nData[0], $this->usernameField => $nData[1] );
