...
class LoginPage extends RunnerPage
{
	...
	function __construct(&$params)
	{
		...
		$this->twoFactAuth = GetGlobalData("bTwoFactorAuth", false) && !inRestApi();
	...
	protected function sendCodeBySMS()
	{
		...
		else
		{
			...
			$this->message = str_replace( "%phone%", $this->getMaskedPhone(), "Uma mensagem de texto com o seu código foi enviada para: %phone%" );
		}
	...
	protected function reportSecondAuthStepResult( $verified )
	{
		...
		if( !$verified )
			$returnJSON["message"] = "Código errado";
		else
		{
			if( $this->mode == LOGIN_POPUP )
				$returnJSON["message"] = "Logado com sucesso.";
	...
	protected function getSelectSQL( $skipPasswordCheck, $strUsername, $strPassword, $loginSet, $cipherer )
	{
		...
		return "select * from ".$this->connection->addTableWrappers("login")
	...
	public function LoginWithSP( $plugin, $token, $addNewUser = true )
	{
		...
		$sqlHead = "select * from ".$this->connection->addTableWrappers( "login" ) ." where ";
		$sql = array();
		if( $userIdField ) {
			$sql []= $sqlHead
		...
		else if( $addNewUser )
		{
			...
			$blobfields = array();
			$retval = DoInsertRecord( "login", $data, $blobfields, $this );
			if( !$retval )
				return false;
	...
	protected function assignBody()
	{
		...
		$this->body["end"] .= "<script type=\"text/javascript\" src=\"".GetRootPathForResources("include/runnerJS/RunnerAll.js?35870")."\"></script>";
	...
	public function doCommonAssignments()
	{
		...
		$rememberbox_checked = "";
		if( $this->rememberPassword || @$_COOKIE["token"] )
